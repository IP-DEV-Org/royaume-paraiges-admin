...
- **Sélecteur de période Jour/Semaine/Mois** (`src/components/period-range.tsx`) — **seule** implémentation des conversions date ↔ inputs HTML5 (`date`/`week`/`month`), du décalage ‹ › et des bornes de période (semaine = **ISO 8601 lundi→dimanche**, ne jamais recalculer localement). Trois briques : `<PeriodRange>` (layout inline complet, utilisé par `/analytics`), `<PeriodModeToggle>` + `<PeriodDateNav>` (composables, utilisées par la sidebar de `/reconciliation`). Exports : `getPeriodBounds`, `shiftPeriod`, `isPeriodAtMax`, `formatPeriodLabel`, `todayUtcISO`, type `PeriodMode`. Remplace l'ancien `src/components/analytics/timeline-period-range.tsx` (supprimé juin 2026).
