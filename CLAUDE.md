...
- **`profiles.is_super_admin`** (seed : `direction@ipdev.lu`) : un super admin peut activer/désactiver l'accès **par page** aux autres admins via l'onglet **Administrateurs** de `/settings` (matrice de switches, onglet visible super admin uniquement, deep-link `?tab=admins`). Colonne **immuable côté client** (trigger anti-escalade `trg_protect_is_super_admin`, promotion uniquement en SQL service_role/postgres).
